...
Counter-Strike soulève sans cesse des questions quand on y joue : un placement, une arme, un timing, une dépense d'argent. Beaucoup d'intuitions s'accumulent partie après partie sans jamais être vérifiées. Nous avons regroupé notre exploration en quatre parties. Nous ouvrons sur une étude de cas approfondie d'une carte (**Mirage**), puis nous élargissons à trois familles de facteurs qui décident d'un round : le contrôle des **sites et de la bombe**, l'**arsenal et l'économie**, et enfin les **croyances de joueurs**. L'objectif est de montrer que l'équilibre de CS ne tient pas d'une symétrie parfaite, mais d'un compromis entre styles de jeu et phases de round.
...
library(dplyr)
library(ggplot2)
donnes <- read_csv("data/grenade_exploded.csv")
...
library(ggplot2)
library(tidyr)
donnes2 <- read_csv("data/grenade_planted_repartition.csv")
...
```{r, fig.width=12, fig.height=8}
smokes <- read_csv("data/smokes_all.csv")
win_rate_per_round <- read_csv("data/win_rate_per_round.csv")
...
library(dplyr)
library(ggplot2)
dataset <- read_csv("data/Result - 1 2026-04-20 23-52-40.csv")
...
```{r}
dataset <- read_csv("data/grenade_emeline 2026-05-16 20-22-34.csv")
...
donnes <- read_csv("data/grenade_emeline 2026-05-16 20-22-34.csv")
...
```{r, message=FALSE}
pistol <- read_csv("data/line_pistol.csv")
...
```{r, message=FALSE}
maps <- read_csv("data/avantage_map.csv")
...
```{r}
kills <- read_csv("data/kills_all.csv")
kills_summary <- kills %>%
...
```{r}
kills <- read_csv("data/kills_all_2.csv")
kills_summary <- kills %>%
mutate(same_side = winner_side == killer_side) %>%
count(same_side) %>%
mutate(pct = n / sum(n))
...
```{r, fig.width=12, fig.height=8}
purchases <- read_csv("data/weapon_buys.csv")
...
```{r, fig.width=12, fig.height=8}
purchases <- read_csv("data/weapon_kills.csv")
...
```{r, message=FALSE}
prec <- read_csv("data/precision_mouvement.csv")
...
```{r, message=FALSE}
timing <- read_csv("data/timing_kills.csv")
...
```{r, , eval=FALSE}
clutchs <- read_csv("data/rclutch.csv")
...
À travers cette base de données, nous avons pu examiner les différents aspects qui composent le jeu Counter-Strike 2. Par l'analyse des pratiques des joueurs, sur les clutch et le pistol round, ou encore leurs préférences d'armes et d'utilisation de la bombe, nous avons pu comprendre à quoi correspondait un profil de joueur moyen (professionnel) dans ses choix, ses capacités et l'impact de ces derniers. Également la diversité que renferme le jeu, notamment sur les choix des armes et grenades durant les parties, et celles qui sont les plus mortelles. Cette étude permet, par ailleurs, de constater que le jeu est très équilibré, que les camps de Terroristes et de Contre-Terroristes se trouvent avec une égalité de chances de victoire, même s'il existe une légère tendance pour un camp ou pour l'autre suivant les cartes. Un travail plus centré sur Mirage met en avant la complexité des cartes, avec les nombreuses lignes de tir possibles et différentes localisations des tueurs et tués. Notre panel de Dataviz est large et couvre même des pratiques moins concrètes comme la superstition liée aux poulets de la carte Inferno ou les pratiques écrites des joueurs.

Nous avons pu réaliser ces graphiques à partir d'un échantillon de 522 matchs, avec des données sur 8 cartes différentes. Cette large base permet donc de considérer que nos graphiques sont suffisamment représentatifs pour être considérés comme fiables, sauf pour la carte Train avec uniquement 10 entrées, ce qui est trop peu. Un projet de plus grande ampleur est totalement faisable et serait très intéressant pour approfondir les recherches d'informations et de schémas sur le jeu, avec une base de données encore plus grande pour obtenir la plus haute fiabilité possible.

Deux logiciels ont été manipulés : RStudio pour la grande majorité des réalisations de Dataviz, ainsi que la Shiny App pour laquelle on code nos instructions pour les graphiques en R ; et en second Power BI, application où l'on vient sélectionner les options de représentation et d'ajustement jusqu'à obtenir notre graphique final.

## Un mot de chaque membre

**Pierre :** Ce projet m'a particulièrement passionné, de la mise en place du pipeline à la conception des algorithmes de machine learning, jusqu'à l'analyse des résultats pour en tirer des conclusions pertinentes. J'ai trouvé cette expérience extrêmement enrichissante, et j'aurais aimé pouvoir approfondir davantage l'exploration de ces données. Les perspectives offertes par ce type de dataset sont immenses : on pourrait, par exemple, entraîner des algorithmes pour développer des bots performants ou concevoir des générateurs de stratégies en temps réel. Cependant, pour aller plus loin, il serait essentiel d'enrichir le dataset avec des données supplémentaires (historique plus long, variables externes, etc.) afin de dégager de vraies tendances et non plus des conjectures. Cette approche permettrait de valider plus robustement les hypothèses et d'ouvrir la voie à des applications plus ambitieuses. Enfin, ce projet m'a permis de manipuler un volume important de données, une expérience que j'ai grandement appréciée et qui a renforcé mon intérêt pour le big data, l'analyse prédictive et l'intelligence artificielle.

**Emeline :** J'ai énormément apprécié travailler sur ce projet, avec la manipulation d'une base de données aussi large qui est une première, ainsi que sur RStudio et Power BI. Cela m'a permis de découvrir le fonctionnement concret de la dataviz, et la nécessité de prévoir la visualisation que l'on souhaite réaliser, même en ignorant le résultat, pour ne pas se perdre dans les possibilités presque infinies qui s'offrent à nous, surtout pour Power BI. J'aurais apprécié d'avoir une partie explicative pour nous aider à comprendre comment récupérer des données sur le sujet de notre choix, de manière générale, avec différents outils que l'on peut utiliser ; ce serait le seul aspect qu'il me manque sur le projet.

**Nicolas :** Ce qui m'a marqué, c'est l'importance du travail en amont pour fournir à la visualisation des données propres, cohérentes avec le message que l'on veut faire passer.

**Magdalena :** Ce que j'ai le plus aimé dans ce projet, c'est d'avoir un dataset aussi riche à explorer : en tant que joueuse, c'était passionnant de pouvoir formuler mes propres questions sur le jeu et d'aller y chercher des réponses dans les données. La partie R a été plus difficile pour moi, mais c'est justement ce qui m'a fait progresser, et je ressors de ce projet avec une bien meilleure aisance sur la manipulation et la visualisation de données. Avec un dataset pareil, on s'est peut-être même un peu perdus tant on a multiplié les questions, et avec du recul j'aurais aimé avoir le temps d'en creuser certaines davantage ou mieux structurer nos recherches plutôt que d'en survoler autant.
...
En ce qui concerne la partie Exploration de la base de données, nous avons opté pour faire entre 3 et 4 questions chacun, au choix et en fonction de nos affinités avec les questions.

**Pierre :**

- Est-ce qu'une carte favorise un camp par rapport à l'autre ?
- Est-ce que les messages dans le chat ont une influence sur la partie ?

**Emeline :**

- La grenade est-elle plus utilisée par les Terroristes ou les Contre-Terroristes ?
- Sur chaque carte, quel site mène le plus souvent à la victoire des Terroristes ?
- Tuer un poulet sur Inferno est-il corrélé à la victoire du round ?

**Nicolas :**

- La pose d'une smoke sur une bombe augmente-t-elle la probabilité de victoire des Terroristes ?
- Le premier kill d'un round est-il décisif ?
- Quelles sont les armes les plus utilisées ?

**Magdalena :**

- Quelle est la probabilité de gagner un clutch selon le désavantage numérique (1v5, 1v4, etc.) ?
- Le "run and gun" est-il efficace, et pour quelles armes ?
- À quel moment du round tue-t-on, selon l'arme ?
- Avantage structurel T vs CT par carte
- Le pistol round confère-t-il un avantage durable sur les rounds suivants ?

La réalisation des différents rendus (présentation, rapports, etc.) a été faite par le groupe Long, sans répartition particulière.
